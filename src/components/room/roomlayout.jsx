...
export default function RoomLayout({ dictionary, dataRoom, lang, expandBookNav }) {
    ...
    // SWIPE HANDLER: chiude il carrello gold con swipe giù
    const handleTouchStart = (e) => {
        touchStartY.current = e.touches[0].clientY
    }
    const handleTouchEnd = (e) => {
        const deltaY = e.changedTouches[0].clientY - touchStartY.current
        // Se l'utente trascina il dito verso il basso di almeno 50px → chiudi e torna su
        if (deltaY > 50 && scrollRef.current) {
            scrollRef.current.scrollTo({ top: 0, behavior: "smooth" })
            setIsInGoldZone(false)
        }
    }
    ...
    return (
        <BubbleFrame bubbleClass="p-0" divBubbleCls={`h-dvh`}>
            <div ref={scrollRef} className="w-full h-full overflow-y-auto snap-y snap-proximity scroll-smooth no-scrollbar overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <div className="w-full h-full sticky top-0 snap-start">
                    ...
                    <div className={`flex absolute top-0 left-0 w-full h-full items-center justify-center z-20 transition-all duration-500 
                        ...
                        <div className={`flex flex-col w-auto xs:w-100 md:w-150 lg:w-min items-center gap-10 mt-[-150px] lg:mt-0 
                            ...
                            <div className={`flex flex-col gap-3 items-center lg:items-start
                                ...
                                {/* DESK */}
                                <p className={`hidden lg:flex text-nowrap ${!isGalleryDesk ? "" : "hidden"}
                                                ${!isInGoldZone ? "" : "hidden"}
                                `}>
                                    {dataRoom.roomSubName}
                                </p>

                                <MasterTitle className="md:text-7xl lg:text-7xl">
                                ...
                                {/* SMARTPHONE */}
                                <p className={`lg:hidden text-[10px] xxs:text-xs ${!isGalleryDesk ? "" : "hidden"}
                                                ${!isInGoldZone ? "" : "hidden"}
                                `}>
                                    {dataRoom.roomSubName}
                                </p>
                    ...
                    <motion.div
                        className="flex flex-col px-5  xs:px-10  md:px-30 gap-3 w-full  absolute bottom-10 xs:bottom-20 md:bottom-20 left-1/2 -translate-x-1/2 justify-center items-center lg:hidden z-99"
                        animate={{
                ...
                <div
                    ref={goldRef}
                    className={`flex flex-col justify-end w-full md:h-200
                        snap-start z-40 rounded-3xl xl:rounded-[40px] pointer-events-none lg:hidden
                        ${isService ? " h-180 xs:h-210" : " h-20 xs:h-180 md:h-100"}`}
                ...
                >
                    ...
                    <AnimatePresence>
                        {shouldShowCart && (
                            <motion.div
                                key="gold-content"
                                className={`relative w-full p-4 flex flex-col items-center h-dvh  md:pb-0 md:px-15 pointer-events-none bg-gold rounded-3xl xl:rounded-[40px]
                                        ${isService ? "md:h-160" : " md:h-140"}
                                        `}
